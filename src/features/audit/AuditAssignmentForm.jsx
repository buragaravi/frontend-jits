...
const AuditAssignmentForm = ({ onClose, onSuccess }) => {
  ...
  const fetchFaculty = async () => {
    try {
      const response = await axios.get('https://backend-jits.onrender.com/api/users', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('https://backend-jits.onrender.com/api/audit/assignments', payload, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });

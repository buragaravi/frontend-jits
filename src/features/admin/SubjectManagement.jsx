...
const SubjectManagement = () => {
  ...
  const fetchSubjects = async () => {
    try {
      ...
      const response = await fetch(`https://backend-jits.onrender.com/api/subjects?${queryParams}`, {
        headers: {
  ...
  const fetchCourses = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://backend-jits.onrender.com/api/courses', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch('https://backend-jits.onrender.com/api/subjects/analytics', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingSubject ? `https://backend-jits.onrender.com/api/subjects/${editingSubject._id}` : 'https://backend-jits.onrender.com/api/subjects';
  ...
  const handleDelete = async (subjectId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://backend-jits.onrender.com/api/subjects/${subjectId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'

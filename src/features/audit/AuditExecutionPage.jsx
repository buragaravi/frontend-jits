...
const AuditExecutionPage = ({ assignment, onBack, onUpdate }) => {
  ...
  const updateItemStatus = async (itemId, status, actualQuantity, remarks) => {
    ...
    try {
      const response = await axios.put(
        `https://backend-jits.onrender.com/api/audit/executions/${execution._id}/items/${itemId}`,
        {
          status,
  ...
  const completeExecution = async () => {
    ...
    try {
      const response = await axios.post(
        `https://backend-jits.onrender.com/api/audit/executions/${execution._id}/complete`,
        {

...
const RequirementsList = ({ userRole, userId, labId, refreshTrigger, onViewRequirement, onRefresh }) => {
  ...
  const fetchRequirements = async () => {
    ...
    try {
      ...
      const response = await axios.get(`https://backend-jits.onrender.com/api/requirements?${params}`, {
  ...
  const handleStatusUpdate = async (requirementId, status, comment = '') => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `https://backend-jits.onrender.com/api/requirements/${requirementId}/status`,
        { status, comment },
        {
          headers: {

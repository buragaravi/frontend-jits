...
const RequirementDetail = ({ requirement, onClose, onUpdate }) => {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://backend-jits.onrender.com/api/requirements/${requirement._id}/status`, {
        method: 'PUT',
        headers: {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://backend-jits.onrender.com/api/requirements/${requirement._id}/comment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleManualConvertToQuotation = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://backend-jits.onrender.com/api/requirements/${requirement._id}/convert-to-quotation`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`

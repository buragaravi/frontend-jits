...
const AdminAuditDetailPage = ({ onBack, assignmentId }) => {
  ...
  const fetchAssignmentDetails = async () => {
    try {
      ...
      const assignmentRes = await axios.get(`https://backend-jits.onrender.com/api/audit/assignments/${assignmentId}`, {
      ...
      if (assignmentRes.data.data?.status === 'in_progress' || assignmentRes.data.data?.status === 'completed') {
        try {
          const executionRes = await axios.get(`https://backend-jits.onrender.com/api/audit/executions/assignment/${assignmentId}`, {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
          });

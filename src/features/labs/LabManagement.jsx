...
const LabManagement = () => {
  ...
  const loadLabs = async () => {
    try {
      ...
      const response = await fetch(
        `https://backend-jits.onrender.com/api/labs?includeInactive=${includeInactive}`,
        {
          headers: {
  ...
  const loadStats = async () => {
    try {
      const response = await fetch('https://backend-jits.onrender.com/api/labs/stats', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = selectedLab ? `https://backend-jits.onrender.com/api/labs/${selectedLab.labId}` : 'https://backend-jits.onrender.com/api/labs';
  ...
  const handleDelete = async (labId) => {
    try {
      const response = await fetch(`https://backend-jits.onrender.com/api/labs/${labId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleToggleActive = async (lab) => {
    try {
      const response = await fetch(`https://backend-jits.onrender.com/api/labs/${lab.labId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleBulkSync = async () => {
    try {
      setIsSyncing(true);
      const response = await fetch('https://backend-jits.onrender.com/api/labs/bulk-sync', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }

...
const CreateGlasswareRequestForm = () => {
  ...
  const createRequestMutation = useMutation({
    mutationFn: async (requestData) => {
      ...
      const response = await axios.post('https://backend-jits.onrender.com/api/glassware/requests', requestData, {
  ...
  const handleGlasswareSearch = async (index, searchTerm) => {
    ...
    try {
      const response = await axios.get(`https://backend-jits.onrender.com/api/glassware/central/available?search=${searchTerm}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

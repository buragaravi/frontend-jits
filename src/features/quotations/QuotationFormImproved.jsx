...
const QuotationForm = ({ userRole, userId, labId }) => {
  ...
  useEffect(() => {
    const fetchAvailableItems = async () => {
      try {
        // Fetch chemicals
        const chemRes = await axios.get('https://backend-jits.onrender.com/api/chemicals/central/available', {
        ...
        // Fetch equipment
        try {
          const equipRes = await axios.get('https://backend-jits.onrender.com/api/equipment/central/available', {
            headers: { Authorization: `Bearer ${token}` },
          });
        ...
        // Fetch glassware
        try {
          const glassRes = await axios.get('https://backend-jits.onrender.com/api/glassware/central/available', {
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = userRole === 'lab_assistant' 
        ? 'https://backend-jits.onrender.com/api/quotations/lab'
        : 'https://backend-jits.onrender.com/api/quotations/central/draft';

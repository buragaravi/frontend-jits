...
const AuditDashboard = ({ userRole = 'admin' }) => {
  ...
  const fetchDashboardData = async () => {
    try {
      const response = await axios.get('https://backend-jits.onrender.com/api/audit/dashboard', {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await axios.get('https://backend-jits.onrender.com/api/audit/assignments', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
